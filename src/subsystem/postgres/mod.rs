...
pub mod migration {
    use {
        crate::config::{Subsystem, Config, DataSource, WithVersion},
    ...
    pub(crate) fn get_effective_timeout(config: &Config, provided_timeout: Option<u64>) -> Option<u64> {
        match &config.subsystem {
            Subsystem::Postgres { timeout, .. } => provided_timeout.or(*timeout),
        }
    }
    ...
    pub(crate) async fn get_db_assets(path: &Path) -> Result<(Config, Pool<Postgres>)> {
        ...
        let pool = match &config.subsystem {
            | Subsystem::Postgres { connection, table, .. } => {
    ...
    pub async fn init(path: &Path) -> Result<()> {
        ...
        match config.subsystem {
            | Subsystem::Postgres { schema, table, .. } => {
                let mut tx = pool.begin().await?;
                {
    ...
    pub async fn up(path: &Path, timeout: Option<u64>, count: Option<usize>, diff: bool) -> Result<()> {
        use {
            crate::{config::Subsystem, migration_diff::{display_migration_diff, parse_migration_operations}},
            std::io::{self, Write},
        };
    ...
    pub async fn down(path: &Path, timeout: Option<u64>, count: Option<usize>, remote: bool, diff: bool) -> Result<()> {
        use {
            crate::{config::Subsystem, migration_diff::{display_migration_diff, parse_migration_operations}},
            std::io::{self, Write},
        };
    ...
    pub async fn new_migration(path: &Path) -> Result<()> {
        use {crate::config::Subsystem, chrono::Utc};
    ...
    pub async fn apply_up(path: &Path, id: &str, timeout: Option<u64>) -> Result<()> {
        use {crate::config::Subsystem, std::io::{self, Write}};
