...
impl MigrationRepository for SqliteRepo {
    async fn init_store(&self) -> Result<()> {
        let mut tx = self.pool.begin().await?;
        {
            ...
            let mut query = sq::build_table_query("CREATE TABLE IF NOT EXISTS ", &self.config.migrations_table());
            ...
            // Create log table
            let mut log_query = sq::build_table_query("CREATE TABLE IF NOT EXISTS ", &self.config.log_table());
    ...
    async fn fetch_applied_ids(&self) -> Result<HashSet<String>> {
        let mut tx = self.pool.begin().await?;
        let ids = sq::get_applied_migrations(&mut tx, &self.config.migrations_table()).await?;
        tx.commit().await?;
        Ok(ids)
    }
    ...
    async fn fetch_last_id(&self) -> Result<Option<String>> {
        let mut tx = self.pool.begin().await?;
        let id = sq::get_last_migration_id(&mut tx, &self.config.migrations_table()).await?;
        tx.commit().await?;
        Ok(id)
    }
    ...
    async fn apply_migration(&self, id: &str, up_sql: &str, down_sql: &str, comment: Option<&str>, pre: Option<&str>, timeout: Option<u64>, dry_run: bool, locked: bool) -> Result<()> {
        ...
        sq::insert_migration_record(&mut *tx, &self.config.migrations_table(), id, up_sql, down_sql, comment, pre, locked).await?;
        
        // Log successful migration
        sq::insert_log_entry(&mut *tx, &self.config.log_table(), id, "up", up_sql).await?;
    ...
    async fn revert_migration(&self, id: &str, down_sql: &str, timeout: Option<u64>, dry_run: bool, unlock: bool) -> Result<()> {
        ...
        let is_locked = sq::is_migration_locked(&mut *tx, &self.config.migrations_table(), id).await?;
        if is_locked && !unlock {
        ...
        sq::delete_migration_record(&mut *tx, &self.config.migrations_table(), id).await?;
        
        // Log successful revert
        sq::insert_log_entry(&mut *tx, &self.config.log_table(), id, "down", down_sql).await?;
    ...
    async fn fetch_history(&self) -> Result<Vec<(String, NaiveDateTime, Option<String>, bool)>> {
        let mut tx = self.pool.begin().await?;
        let map = sq::get_migration_history(&mut tx, &self.config.migrations_table()).await?;
        tx.commit().await?;
    ...
    async fn fetch_recent_for_revert_remote(&self) -> Result<Vec<(String, String)>> {
        let mut tx = self.pool.begin().await?;
        let rows: Vec<SqliteRow> = sq::get_recent_migrations_for_revert(&mut tx, &self.config.migrations_table()).await?;
        tx.commit().await?;
    ...
    async fn fetch_down_sql(&self, id: &str) -> Result<Option<String>> {
        ...
        q.push(migration::quote_ident(&self.config.migrations_table()));
        q.push(" WHERE id = ?");
